...
<div class="space-y-8" x-data="{ showTraspasoCoordModal: false, openDecisionCoordId: null }">
    ...
    <!-- Sección: Alertas de Morosidad y Distribuidoras con 3+ Retrasos en tu Sucursal -->
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-white flex items-center gap-2">
                            Supervisión de Morosidad en tu Sucursal
                        ...
                        <p class="text-slate-400 text-xs mt-0.5">Distribuidoras asignadas a tu sucursal con 3 o más retrasos en sus cortes. Evalúa la suspensión y bloqueo de emisión de vales.</p>
                    </div>
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-800/60">
                        @foreach($distribuidorasMorosasOEnRiesgo as $distM)
                            <tr class="hover:bg-slate-800/40 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-xs font-semibold text-slate-200">{{ $distM->coordinador?->name ?? 'Sin asignar' }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    @if(!$distM->esMorosa())
                                        <button @click="openMorosidadDistId = (openMorosidadDistId === 'marcar_{{ $distM->id }}' ? null : 'marcar_{{ $distM->id }}')"
                                    ...
                                    @else
                                        <button @click="openMorosidadDistId = (openMorosidadDistId === 'desmarcar_{{ $distM->id }}' ? null : 'desmarcar_{{ $distM->id }}')"
                                                class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold shadow-md transition bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-950/20">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- Panel desplegable de confirmación para MARCAR -->
                            @if(!$distM->esMorosa())
                                <tr x-show="openMorosidadDistId === 'marcar_{{ $distM->id }}'" class="bg-slate-950/60" style="display: none;" x-transition>
                                        ...
                                        <form novalidate method="POST" action="{{ route('gerente.distribuidores.decidir-morosidad', $distM) }}" class="space-y-3">
                                            ...
                                            <div class="p-4 rounded-xl border bg-rose-950/20 border-rose-500/30">
                                                ...
                                                <div class="flex justify-end gap-2 mt-3">
                                                    ...
                                                    <button type="submit" onclick="return confirm('¿Confirmas declarar en MOROSIDAD a esta distribuidora? Sus vales pendientes se cancelarán automáticamente.')" class="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold shadow-lg shadow-rose-950/30">
                            ...
                            @else
                                <!-- Panel desplegable de confirmación para DESMARCAR (Quitar Morosidad) -->
                                <tr x-show="openMorosidadDistId === 'desmarcar_{{ $distM->id }}'" class="bg-slate-950/60" style="display: none;" x-transition>
                                    <td colspan="6" class="px-6 py-4">
                                        <form novalidate method="POST" action="{{ route('gerente.distribuidores.decidir-morosidad', $distM) }}" class="space-y-3">
                                            @csrf
                                            <input type="hidden" name="accion" value="desmarcar">
                                            
                                            <div class="p-4 rounded-xl border bg-emerald-950/20 border-emerald-500/30">
                                                <h4 class="text-sm font-bold text-white mb-1">
                                                    ¿Retirar Estado de Morosidad a {{ $distM->name }}?
                                                </h4>
                                                <p class="text-xs text-slate-400">
                                                    Al retirar la morosidad, la distribuidora será reactivada y podrá volver a emitir y colocar vales a sus clientes.
                                                </p>
                                                ...
                                                <div class="flex justify-end gap-2 mt-3">
                                                    ...
                                                    <button type="submit" onclick="return confirm('¿Confirmas RETIRAR el estado de morosidad a esta distribuidora? Podrá emitir vales nuevamente.')" class="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-lg shadow-emerald-950/30">
                                                        Confirmar y Quitar Morosidad
                                                    </button>
                                                </div>
                                            </div>
                                        </form>
    ...
    <!-- SECCIÓN: Solicitudes de Aumento de Crédito Pendientes de tu Sucursal -->
        ...
        <div class="bg-slate-900 border border-emerald-500/40 rounded-2xl shadow-xl overflow-hidden" x-data="{ openDecisionCreditoId: null }">
            <div class="p-6 border-b border-slate-800 bg-emerald-950/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-white flex items-center gap-2">
                            Solicitudes de Incremento de Crédito en tu Sucursal
                        ...
                        <p class="text-slate-400 text-xs mt-0.5">Solicitudes enviadas por tus coordinadores para ampliar la línea de crédito de distribuidoras.</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="p-5 space-y-3">
                        ...
                        <div x-show="openDecisionCreditoId === '{{ $sc->id }}'" class="p-4 bg-slate-950/70 border border-slate-800 rounded-xl space-y-3" style="display: none;" x-transition>
                            <form novalidate method="POST" action="{{ route('solicitudes-credito.procesar', $sc) }}" class="space-y-3">
                                @csrf
                                <input type="hidden" name="accion" id="dec_cred_gs_{{ $sc->id }}">
                                <div>
                                ...
                                <div class="flex justify-end gap-2">
                                    <button type="submit" onclick="document.getElementById('dec_cred_gs_{{ $sc->id }}').value = 'rechazar'; return confirm('¿Rechazar solicitud de incremento de crédito?')" class="inline-flex items-center gap-1 px-4 py-2 rounded-xl bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 border border-rose-500/30 text-xs font-bold transition">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                        Rechazar
                                    </button>
                                    <button type="submit" onclick="document.getElementById('dec_cred_gs_{{ $sc->id }}').value = 'aprobar'; return confirm('¿Aprobar incremento de crédito a ${{ number_format($sc->limite_nuevo, 2) }}?')" class="inline-flex items-center gap-1 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg text-xs font-bold transition">
    ...
    <!-- SECCIÓN: Solicitudes de Ascenso de Categoría Pendientes de tu Sucursal -->
        ...
        <div class="bg-slate-900 border border-amber-500/40 rounded-2xl shadow-xl overflow-hidden" x-data="{ openDecisionCatId: null }">
            <div class="p-6 border-b border-slate-800 bg-amber-950/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-white flex items-center gap-2">
                            Solicitudes de Ascenso de Categoría en tu Sucursal
                        ...
                        <p class="text-slate-400 text-xs mt-0.5">Propuestas de cambio de nivel para distribuidoras de tu sucursal.</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="p-5 space-y-3">
                        ...
                        <div x-show="openDecisionCatId === '{{ $scat->id }}'" class="p-4 bg-slate-950/70 border border-slate-800 rounded-xl space-y-3" style="display: none;" x-transition>
                            <form novalidate method="POST" action="{{ route('solicitudes-categoria.procesar', $scat) }}" class="space-y-3">
                                @csrf
                                <input type="hidden" name="accion" id="dec_cat_gs_{{ $scat->id }}">
                                <div>
                                ...
                                <div class="flex justify-end gap-2">
                                    <button type="submit" onclick="document.getElementById('dec_cat_gs_{{ $scat->id }}').value = 'rechazar'; return confirm('¿Rechazar solicitud de ascenso de categoría?')" class="inline-flex items-center gap-1 px-4 py-2 rounded-xl bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 border border-rose-500/30 text-xs font-bold transition">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                        Rechazar
                                    </button>
                                    <button type="submit" onclick="document.getElementById('dec_cat_gs_{{ $scat->id }}').value = 'aprobar'; return confirm('¿Aprobar ascenso a categoría {{ strtoupper($scat->categoria_nueva) }}?')" class="inline-flex items-center gap-1 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg text-xs font-bold transition">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                        Aprobar Categoría
                                    </button>
                                </div>
