...
<div class="space-y-8">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-slate-800 p-6 sm:p-8 shadow-2xl">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                ...
                <p class="text-slate-400 text-sm mt-1">Supervisión de cartera, préstamos por distribuidora y autorizaciones de tu sucursal.</p>
            </div>
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <a href="{{ route('solicitudes-clientes.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-bold transition">
                    ...
                    Solicitudes de mi Sucursal ({{ $totalSolicitudesPendientes }})
                </a>
    ...
    <!-- KPIs de Cartera Activa de la Sucursal -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
            ...
            <p class="text-xs text-indigo-400 mt-1">En curso en tu sucursal</p>
        </div>
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
            <span class="text-xs font-semibold text-indigo-400 uppercase tracking-wider">Capital Prestado</span>
            <div class="text-2xl font-black text-white mt-2">${{ number_format($statsPrestamos['monto_prestado'], 2) }}</div>
            <p class="text-xs text-slate-400 mt-1">Colocación vigente</p>
        </div>
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
            ...
            <p class="text-xs text-slate-400 mt-1">Adeudo pendiente en sucursal</p>
        </div>
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl">
            ...
            <p class="text-xs text-slate-400 mt-1">Cobranza acumulada</p>
        </div>
    </div>

    <!-- Sección: Solicitudes Pendientes de su Sucursal -->
        ...
        <div class="p-6 border-b border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h2 class="text-lg font-bold text-white flex items-center gap-2">
                    ...
                    Solicitudes de Clientes de tu Sucursal (Pendientes)
                </h2>
                <p class="text-slate-400 text-xs mt-0.5">Autoriza o rechaza solicitudes de modificación y baja emitidas por tus distribuidores.</p>
            </div>
            <a href="{{ route('solicitudes-clientes.index', ['estado' => 'pendiente']) }}" class="text-xs font-semibold text-indigo-400 hover:text-indigo-300 transition">
                Ver todas las de mi sucursal &rarr;
            </a>
        </div>
                ...
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-950/60 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-800">
                        <tr>
                            ...
                            <th class="px-6 py-3.5">Distribuidor</th>
                        ...
                        @foreach($solicitudesPendientes as $sol)
                            <tr class="hover:bg-slate-800/40 transition">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-xs text-slate-400">{{ $sol->distribuidor?->email }}</div>
                                </td>
                                <td class="px-6 py-4">
